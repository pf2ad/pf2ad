...
if [ "$(cat /etc/version)" != "2.3.3-RELEASE" ]; then
	echo "ERROR: You need the pfSense version 2.3.3 to apply this script"
	exit 2
fi
...
/usr/sbin/pkg lock pfSense-2.3.3
...
/usr/sbin/pkg unlock pfSense-2.3.3
...
fetch -o /usr/local/pkg -q https://pkg.mundounix.com.br/pfsense/2.3.3-samba4/samba/samba.inc
fetch -o /usr/local/pkg -q https://pkg.mundounix.com.br/pfsense/2.3.3-samba4/samba/samba.xml
...
cd /usr/local/pkg
if ! fetch -o - -q https://pkg.mundounix.com.br/pfsense/2.3.3-samba4/samba/squid_winbind_auth.patch | patch -p0 --dry-run -t | grep "Reversed"; then
    fetch -o - -q https://pkg.mundounix.com.br/pfsense/2.3.3-samba4/samba/squid_winbind_auth.patch | patch -b -p0
fi
fetch -o /usr/local/pkg -q https://pkg.mundounix.com.br/pfsense/2.3.3-samba4/samba/squid.inc
